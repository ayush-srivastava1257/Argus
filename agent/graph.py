...
def build_investigation_graph():
    ...
    Flow:
        parse_query → create_plan → execute_tools → synthesize_findings
                                                         ↓
                                              [if HIGH] → generate_sar → END
                                              [else]    → END
    """
    ...
    # Conditional: SAR generation only for HIGH risk
...
def run_investigation(user_query: str) -> dict:
    ...
    result = investigation_agent.invoke(initial_state)
    return result
